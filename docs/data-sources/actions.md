...
- `event_report_config` (Attributes) Event report action configurations. Available in the Enterprise edition. (see [below for nested schema](#nestedatt--actions--options--event_report_config))
...
- `attach_event_report` (Boolean) If enabled, the event report generated by a previous event report action is attached as a compressed CSV file. Available in the Enterprise edition.
- `attachments` (List of String) Paths to attach. The total size is limited to 10 MB.
- `bcc` (List of String) List of BCC email addresses.
...
<a id="nestedatt--actions--options--event_report_config"></a>
### Nested Schema for `actions.options.event_report_config`

Read-Only:

- `fs_actions` (List of String) Filter events by filesystem action. If empty, all actions are included.
- `split_reports` (Boolean) If enabled, separate events are generated for each user, allowing subsequent actions to be executed individually per user.
- `statuses` (List of Number) Filter events by status: 1=OK, 2=error, 3=quota exceeded. If empty, all statuses are included.
- `time_window` (Number) Time window in minutes. The query range is [now - time_window, now].
...
- `continue_on_error` (Boolean) Continue processing remaining files after a failure instead of stopping at the first error. Currently supported for copy actions only. Available in the Enterprise edition.
...
- `max_retries` (Number) Number of retry attempts per file. 0 means no retry. Available in the Enterprise edition.
- `on_source_copied` (Number) Source disposition after successful copy. 0 = none (default), 1 = delete source, 2 = move source to on_source_copied_move_path. Available in the Enterprise edition.
- `on_source_copied_move_path` (String) Destination directory for moved sources. Required when on_source_copied is 2. Available in the Enterprise edition.
...
- `metadata` (Attributes) Metadata key/value pair to verify. (see [below for nested schema](#nestedatt--actions--options--fs_config--metadata_check--metadata))
- `path` (String) Virtual path to check.
- `timeout` (Number) Timeout in seconds for periodic retry.
...
- `value` (String) Expected metadata value. If empty, the key is expected to be absent.
...
- `body` (String) Body content as text for this multipart part.
- `filepath` (String) Path to the file to be sent as an attachment.
- `headers` (Attributes List) Additional headers for this part. (see [below for nested schema](#nestedatt--actions--options--http_config--parts--headers))
- `name` (String) Name of the multipart form field.
