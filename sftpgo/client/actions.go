...
const (
	...
	ActionTypeEventReport          // Enterprise
)
...
type EventActionEmailConfig struct {
	...
	// If enabled, the event report generated by a previous event report action
	// is attached as a compressed CSV file
	AttachEventReport bool `json:"attach_event_report,omitempty"`
}
...
// CopyConfig defines the configuration for a filesystem copy
type CopyConfig struct {
	// key is the source and target the value
	KeyValue
	// Source disposition after successful copy.
	// 0 = none (default), 1 = delete source, 2 = move source to OnSourceCopiedMovePath
	OnSourceCopied int `json:"on_source_copied,omitempty"`
	// Destination directory for moved sources. Required when OnSourceCopied is 2.
	// The directory structure relative to source base is preserved
	OnSourceCopiedMovePath string `json:"on_source_copied_move_path,omitempty"`
	// Number of retry attempts per file. Each retry waits 5 seconds. 0 means no retry
...
type EventActionFilesystemConfig struct {
	...
	Copy []CopyConfig `json:"copy,omitempty"`
	// Continue processing remaining files after a failure instead of stopping at
	// the first error. Currently supported for copy actions only
...
// EventActionEventReportConfig defines the configuration for an event report action
type EventActionEventReportConfig struct {
	// Filter events by filesystem action (e.g. upload, download, delete).
	// If empty, all actions are included
	FsActions []string `json:"fs_actions,omitempty"`
	// Filter events by status: 1=OK, 2=error, 3=quota exceeded.
	// If empty, all statuses are included
	Statuses []int32 `json:"statuses,omitempty"`
	// Time window in minutes. The query range is [now - TimeWindow, now]
	TimeWindow int `json:"time_window"`
	// If enabled, separate events are generated for each user, allowing subsequent
	// actions to be executed individually per user
...
type EventActionOptions struct {
	...
	EventReportConfig    EventActionEventReportConfig   `json:"event_report_config"`
}
